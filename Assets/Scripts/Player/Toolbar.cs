    ...
    public void DropItemsFromSlot(int slotIndexValue)
    {
        if(player != null && slots[slotIndexValue].itemSlot.stack != null)
        {
            ...
            for (int i = 0; i < amount; i++) // for each item in slot
                controller.SpawnVoxelRbAtPos(new Vector3(player.transform.position.x, player.transform.position.y, player.transform.position.z) + player.transform.forward * 4, slots[slotIndexValue].itemSlot.stack.id);
